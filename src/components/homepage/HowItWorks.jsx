        ...
        <div className="flex flex-row flex-wrap -mx-3">
          ...
          <div className="lg:w-1/2 lg:mt-0 mt-8 max-lg:flex max-md:flex-col w-full px-3">
            {WorkData.map((obj, i) => (
              ...
              >
                <div className={`w-12 h-[41px] bg-lightBlue flex justify-center items-center max-sm:w-[39px] max-sm:h-[33px] ${i=== 1 ? "animate-wiggle2":"animate-wiggle3"} `}><img src={obj.icons} alt="locate" className="max-sm:w-[18px]" /></div>
                <h3 className="pt-4 max-sm:pt-2 font-normal sm:text-2xl text-xl mb-[6px] text-darkBlue leading-sm">
                  {obj.heading}
                </h3>
                <p className={`font-normal lg:max-w-[496px] max-sm:leading-md max-xs:max-w-[289px] text-lightBlack sm:text-base text-sm ${i=== 1 ? "max-w-[275.59px]":""}`}>
                  {obj.desc}
                </p>
              </div>
            ))}
